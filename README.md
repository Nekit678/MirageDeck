...
- формирование аппаратных пакетов `ACK...OK` для клавиш/энкодеров и
  фазовых `TP`-пакетов для touch bar;
...
5. В Button Mode четыре области сенсорного экрана соответствуют четырём
   ...
   тап передаётся как `DOWN`/`UP` и запускает элемент, а горизонтальный drag —
   как `DOWN`/`MOVE`/`UP`. Каждый контакт получает возрастающий sequence,
   поэтому новый жест можно начинать в любом месте панели без скачка содержимого.
